 ...
 * Date: 3/12/2015
 *
 * run extension with 3 command-line arguments:
 * first is a text file containing the movie casts.
 * second argument is an output text file containing the actors and movies.
 * third argument is a number between 1 and 100 which represents the
 *     percentage of actors you want to see.
...
int main(int argc, char const *argv[]){ 
  ...
  double percent = stoi(argv[3]);
  if(percent > 100 || percent<=0){
    cout<<"percent out of bound"<<endl;
    return -1;
  }

  int actorToSee = (int)(index*percent/100);
  ...
  while(actorDs.numOfSets() > 1){
    ...
    set<string>::iterator itActorPool;
    int actorMinus = 0;
    for(int i = 0; i < actorList.size(); ++i){   
      itActorPool = actorPool.find(actorList[i]);
      if(itActorPool == actorPool.end()){
      	actorPool.insert(actorList[i]);
      	actorMinus++;
      }
      outfile<<actorList[i]<<'\t'<<movieNameR<<'\t'<<year<<endl; 
    }   

    actorToSee = actorToSee - actorMinus;
    cout<<actorToSee<<endl;
    if(actorToSee <= 0){
    	break;
    }
